...
#define SWITCH_PRIO 14
#define BUTTON_PRIO 16

// Task Periods

#define CONTROL_PERIOD  30 //0.1s
#define VEHICLE_PERIOD  30 //0.1s
...
return IORD_ALTERA_AVALON_PIO_DATA(DE2_PIO_HEX_HIGH28_BASE); 
}
...
void show_position(INT16U position){
	...
	if(position==24000){
		 ...
		 IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_HEX_HIGH28_BASE, hexread|((pose[5])<<14));
	}
	else{	
	...
	/*		We turn on the desired led
		we also need to keep the other leds on, that why we use the Bitwise operation		*/
			case 0: IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_REDLED18_BASE,((1<<17)|(switches_pressed()&3))); 
					IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_HEX_HIGH28_BASE, ~(hexread|((pose[0])<<14)));
					globpose=0;
					break;
			case 1: IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_REDLED18_BASE,((1<<16)|(switches_pressed()&3)));
					IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_HEX_HIGH28_BASE, ~(hexread|((pose[1])<<14)));
					globpose=1;
					break;
			case 2: IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_REDLED18_BASE,((1<<15)|(switches_pressed()&3)));
					IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_HEX_HIGH28_BASE, ~(hexread|((pose[2])<<14)));
					globpose=2;
					break;
			case 3: IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_REDLED18_BASE,((1<<14)|(switches_pressed()&3)));
					IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_HEX_HIGH28_BASE, ~(hexread|((pose[3])<<14)));
					globpose=3;
					break;
			case 4: IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_REDLED18_BASE,((1<<13)|(switches_pressed()&3)));
					IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_HEX_HIGH28_BASE, ~(hexread|((pose[4])<<14)));
					globpose=4;
					break;
...
void ControlTask(void* pdata)
{
  ...
  while(1)
    { 
	  ...
	  if(( engine == off ) && (*current_velocity<=0)){ //If the speed is zero, and the engine is off, everything turns off
			...
			if((gas_pedal == on) && (throttle <80)) 
			 //if the gas pedal is on we increase the throttle, but keeping it under 80
				throttle=throttle+5;
			  
		else if((gas_pedal == off)){ //if the pedal is off, we try to keep the throttle stable to 4V
			    if(throttle<40){
					throttle+=5;
				}
				if(throttle>=40){
  		 ...
  		 if( (top_gear == on) && (IORD_ALTERA_AVALON_PIO_DATA(DE2_PIO_GREENLED9_BASE)==4) && (( *current_velocity <=250) && (*current_velocity>180)) ){ //these are the condition to keep the cruise control on
				cruise_control=on;	
